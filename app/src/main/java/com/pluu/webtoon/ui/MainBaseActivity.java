...
public class MainBaseActivity extends AppCompatActivity {
    ...
    private final List<ServiceConst.NAV_ITEM> mNavDrawerItems = new ArrayList<>();
    ...
    protected ServiceConst.NAV_ITEM getSelfNavDrawerItem() {
        return ServiceConst.NAV_ITEM.INVALID;
    }
    ...
    private void populateNavDrawer() {
        ...
        for (ServiceConst.NAV_ITEM item : ServiceConst.NAV_ITEM.values()) {
            if (item.isSelect) {
    ...
    private void createNavDrawerItems() {
        ...
        int i = 0;
        for (ServiceConst.NAV_ITEM item : mNavDrawerItems) {
    ...
    private View makeNavDrawerItem(final ServiceConst.NAV_ITEM item, ViewGroup container) {
        boolean selected = getSelfNavDrawerItem() == item;
        int layoutToInflate;
        if (item == ServiceConst.NAV_ITEM.SEPARATOR) {
    ...
    private void formatNavDrawerItem(View view, ServiceConst.NAV_ITEM item, boolean selected) {
        if (isSeparator(item)) {
            // not applicable
            return;
        }
    ...
    private boolean isSpecialItem(ServiceConst.NAV_ITEM item) {
        switch (item) {
            default:
                return false;
        }
    ...
    private boolean isSeparator(ServiceConst.NAV_ITEM item) {
        return item == ServiceConst.NAV_ITEM.SEPARATOR;
    }
    ...
    private void onNavDrawerItemClicked(final ServiceConst.NAV_ITEM item) {
    ...
    private void goToNavDrawerItem(ServiceConst.NAV_ITEM item) {
        if (item != null) {
     ...
     */
    private void setSelectedNavDrawerItem(ServiceConst.NAV_ITEM item) {
        if (mNavDrawerItemViews != null) {
            for (int i = 0; i < mNavDrawerItemViews.length; i++) {
                if (i < mNavDrawerItems.size()) {
                    ServiceConst.NAV_ITEM thisItem = mNavDrawerItems.get(i);
